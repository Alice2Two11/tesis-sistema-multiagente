...
class TestAgent06Integration(unittest.TestCase):
 def make_project(self):
  ...
  (outputs/'04_outline').mkdir(parents=True);(outputs/'03_thematic_analysis').mkdir();(outputs/'01_rag').mkdir();(outputs/'00_orchestrator_planner').mkdir();(exp/'03_chunks').mkdir();(exp/'04_chroma').mkdir()
 ...
 def test_build_real_execution_without_llm_utils_and_transaction(self):
  ...
  agent,ai,cfg=build_real_draft_execution(root,1,collection_factory=lambda cfg:Collection(),runtime_factory=rf)
